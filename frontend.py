...
def run_config():
    ...
    try:
        ...
        hostname, vlan, backup = configure_switch(
            host, username, password, vlan_id, vlan_name
        )
...
tk.Label(root, text="VLAN ID").grid(row=3, column=0)
vlan_entry = tk.Entry(root)
vlan_entry.grid(row=3, column=1)

tk.Label(root, text="VLAN Name").grid(row=4, column=0)
name_entry = tk.Entry(root)
name_entry.grid(row=4, column=1)

tk.Button(root, text="Configure VLAN", command=run_config).grid(row=5, column=1)
